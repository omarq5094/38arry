...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  loadOnlineVisitors();
  setInterval(loadOnlineVisitors, 30000);
...
async function loadLeads() {
  ...
  try {
    const result = await apiGet("getLeads", { token: getToken() });
    if (!result.ok) throw new Error(result.error || "تعذر تحميل الطلبات");

    const leads = result.data || [];
    status.textContent = leads.length ? `تم تحميل ${leads.length} طلب.` : "لا توجد طلبات حتى الآن.";

    tbody.innerHTML = leads.map(lead => `
      <tr>
        <td>${escapeHtml(lead.property_id || "-")}</td>
...
async function loadOnlineVisitors() {
  const node = document.getElementById("onlineVisitorsCount");
  if (!node) return;

  try {
    const result = await apiGet("getOnlineVisitors", {
      token: getToken()
    });

    if (!result.ok) throw new Error(result.error || "فشل تحميل عدد الزوار");

    node.textContent = result.data.online_visitors || 0;
  } catch (_) {
    node.textContent = "-";
  }
}
