...
async function loadAdminPropertyDetails() {
  ...
  try {
    status.textContent = "جاري تحميل العقار من قائمة المراجعة...";

    const result = await apiGet("getPendingProperties", {
      token: getToken()
    });

    if (!result.ok) throw new Error(result.error || "تعذر تحميل العقارات تحت المراجعة");
    ...
    if (!property) {
      throw new Error("العقار غير موجود ضمن قائمة المراجعة. قد يكون تم قبوله أو رفضه أو تغيّرت حالته.");
    }
    ...
    <div class="review-page-layout">
      <div class="review-main">
        ...
        <div class="actions">
          ...
          ${p.latitude && p.longitude ? `<a class="secondary-btn" href="https://maps.google.com/?q=${encodeURIComponent(p.latitude + "," + p.longitude)}" target="_blank">فتح الإحداثيات</a>` : ""}
        </div>
